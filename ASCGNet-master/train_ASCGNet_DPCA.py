...
from model.ASCGNet_DPCA import ASCGNet_DPCA
...
def get_parser():
    ...
    # parser.add_argument('--config', type=str, default='config/pascal/ascgnet_dpca/ascgnet+DPCA_pascal_split0_vgg16.yaml',help='config file')
    # parser.add_argument('opts', help='see config/pascal/ascgnet_dpca/ascgnet+DPCA_pascal_split0_vgg16.yaml for all options', default=None,nargs=argparse.REMAINDER)

    parser.add_argument('--config', type=str, default='config/tomato/ascgnet_dpca/ascgnet+DPCA_tomato_ascgnet+pascal_split0_vgg16.yaml', help='config file')
    parser.add_argument('opts', help='see config/tomato/ascgnet+DPCA_ablation/ascgnet+DPCA_tomato_ascgnet+pascal_split0_vgg16.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    model = ASCGNet_DPCA(layers=args.layers, classes=args.classes, zoom_factor=8,
    ...
    base_modules = [
        model.down_query, model.down_supp,
        model.init_merge, model.beta_conv, model.inner_cls,  # 原版PFENet的FEM循环模块
        model.color_merge_conv, model.alpha_conv, model.res1, model.res2,  # 颜色加法注入与融合
        model.polar_color_anchor,  # 极坐标颜色锚点生成器
        model.cls  # 分类头 (删掉了 aux_cls，因为我最新代码里辅助损失是直接算的，没有单独的 aux_cls 模块)
    ]
    base_params = []
    for m in base_modules:
        base_params.extend(list(m.parameters()))
...
def train(train_loader, model, optimizer, epoch):
    ...
    for module in model.modules():
        if isinstance(module, torch.nn.BatchNorm2d) or isinstance(module, torch.nn.SyncBatchNorm):
            module.eval()
