# =====================================================================
# Ablation 3: QPCANet_FeatOnly (Baseline + Module 2)
# 创新点: 仅保留 FEM 内部的“特征层颜色注入 (Feature Injection)”
# 去除项: 关闭“空间双重净化 (Spatial Mask Refinement)”，即不做乘法
# =====================================================================
import math
import torch
...
class ASCGNet_DPCA_FeatOnly(nn.Module):
    def __init__(self, layers=50, classes=2, zoom_factor=8, \
        ...
        super(ASCGNet_DPCA_FeatOnly, self).__init__()
        ...
        for idx, bin in enumerate(self.ppm_scales):
            ...
            # 颜色加法注入分支 (保留 Module 2)
            color_branch = nn.Sequential(
                nn.Conv2d(1, reduce_dim, kernel_size=1, bias=False),
                nn.ReLU(inplace=True)
            )
            nn.init.zeros_(color_branch[0].weight)
            self.color_merge_conv.append(color_branch)
        ...
        self.color_merge_conv = nn.ModuleList(self.color_merge_conv)
    ...
    def forward(self, x, s_x=torch.FloatTensor(1, 1, 3, 473, 473).cuda(), s_y=torch.FloatTensor(1, 1, 473, 473).cuda(),
        ...
        # ==============================================================
        # 【核心操作：关闭 Module 1 (SMR)】
        # 不使用 color_anchor_map 进行空间净化乘法，直接传递原版的形状掩码
        refined_corr_mask = corr_query_mask
        # ==============================================================
        ...
        for idx, tmp_bin in enumerate(self.ppm_scales):
            ...
            color_map_bin = F.interpolate(color_anchor_map, size=(bin, bin), mode='bilinear', align_corners=True)

            merge_feat_bin = torch.cat([query_feat_bin, supp_feat_bin, corr_mask_bin], 1)
            merge_feat_bin = self.init_merge[idx](merge_feat_bin)

            # ==============================================================
            # 【保留 Module 2 (FCI)】: 特征层颜色注入
            # 在特征层依然做残差注入
            color_feat_bin = self.color_merge_conv[idx](color_map_bin)
            merge_feat_bin = merge_feat_bin + color_feat_bin
            # ==============================================================
