...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None):
    ...
    for l_idx in tqdm(range(len(list_read))):
        ...
        # label = cv2.imread(label_name, cv2.IMREAD_GRAYSCALE)源文件的代码
        try:
            from PIL import Image
            label_pil = Image.open(label_name)
            label = np.array(label_pil)
        except Exception as e:
            print(f"Error reading label {label_name}: {e}")
            continue
...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, data_list=None, transform=None, mode='train', use_coco=False,
                 use_split_coco=False,use_tomato_list=False):
        ...
        if not use_coco:
            if not use_tomato_list:
                ...
                self.class_list = list(range(21, 22))
                self.sub_list = list(range(21, 22))
                self.sub_val_list = list(range(21, 22))
        else:
            if use_split_coco:
    ...
    def __getitem__(self, index):
        ...
        if 255 in label_class:
            label_class.remove(255)
        if hasattr(self, 'use_tomato_list') and self.use_tomato_list:
            # 番茄数据集专用逻辑
            new_label_class = [c for c in label_class if c == 21]  # 只保留番茄类别
            if len(new_label_class) == 0:
                # 如果没有番茄类别，使用第一个有效类别
                new_label_class = label_class[:1] if label_class else [21]
        else:
            new_label_class = []
            for c in label_class:
        ...
        if len(label_class) == 0:
            # 确保至少有一个类别
            label_class = [21] if hasattr(self, 'use_tomato_list') and self.use_tomato_list else [1]
        ...
        for k in range(self.shot):
            ...
            try:
                from PIL import Image
                label_pil = Image.open(support_label_path)
                support_label = np.array(label_pil)
            except Exception as e:
                print(f"Error reading label {support_label_path}: {e}")
                raise
